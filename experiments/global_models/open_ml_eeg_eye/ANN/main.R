...
for(i in 1:h_size){
  tmp =  evaluate_test_no_em(data_x = data$test$x,data_y = data$test$y,h_glob)
...
hospital_models = lapply(1:h_size,function(x){
  ...
  test = data$test
  h = create_hospital_model()
  tmp_dec = train_model(20,h$decoder,x_data =tmp_data$x ,y_data = tmp_data$y)
  h$decoder = tmp_dec$new_model
  return(h)
})
...
#   y = hospital_models[[x]]
#   y$decoder$save_weights(paste0("agent_",x,".h5"))
# })
...
hospital_models = lapply(1:h_size,function(x){
  ...
  test = data$test
  h = create_hospital_model()
  tmp_dec = train_model(init_epoch/3,h$decoder,x_data =tmp_data$x ,y_data = tmp_data$y)
  h$decoder = tmp_dec$new_model
  return(h)
})
